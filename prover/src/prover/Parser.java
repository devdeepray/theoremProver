...
public class Parser {
	...
	public static TreeSet<ExprTree> deduce(ExprTree result) {
		...
		while(current.proposition == null) {
			...
			hypSet.add(Parser.contraPositive(current.left));
	...
	static String removeAnd(String exprStr) {
		...
		while((dotPos = exprStr.indexOf('.')) != -1)
		{
			...
			//System.out.println(mlexpr);
			
			//System.out.println(mrexpr);
			//System.out.println(rexpr);
			
			exprStr = lexpr + "((" + mlexpr + "->(" + mrexpr + "->F))->F)" + rexpr;
		}
		return exprStr;
	}
	...
	public static ExprTree contraPositive(ExprTree tmp) {
		if(tmp.left == null) return tmp;
		// TODO Auto-generated method stub
		ExprTree newTree = new ExprTree();
		newTree.left = new ExprTree();
		newTree.right = new ExprTree();
		newTree.left.left = tmp.right;
		newTree.left.right = new ExprTree("F");
		newTree.right.left = tmp.left;
		newTree.right.right = new ExprTree("F");
		return newTree;
	}
